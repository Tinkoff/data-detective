...
from mg_airflow.constants import RELATION_KEY_FIELDS, RELATION_NONE, JSON_FIELDS
from mg_airflow.utils.notnull import is_not_empty
...
def filter_for_entity(source_df: DataFrame, context: dict, hook: PostgresHook):
    ...
    dump_hash_mg = hook.get_pandas_df(dump_hash_query.format(**query_params))
    number_of_rows = len(dump_hash_mg.index)
    dump_hash_mg = dump_hash_mg.drop_duplicates(subset=['urn'])
    number_of_unique_rows = len(dump_hash_mg.index)
    if number_of_rows > number_of_unique_rows:
        logging.warning(f"Duplicate rows in mg.entity DAG {context['dag'].dag_id}")
    ...
    source_df = source_df.astype(object).merge(dump_hash_mg, how='outer', on=['urn'], indicator='hash_flg')
    del dump_hash_mg
...
def filter_for_breadcrumb(source_df, context: dict, hook: PostgresHook):
    ...
    source_df = source_df.astype(object)\
        .merge(dump_hash_mg, how='outer', on=['urn'], indicator='hash_flg')
    del dump_hash_mg
    source_df = source_df[(source_df['hash_flg'] != 'both') | (source_df['hash_x'] != source_df['hash_y'])]
    source_df['diff_flg'] = source_df['hash_flg'].map({'left_only': 'I', 'right_only': 'D', 'both': 'U'})
