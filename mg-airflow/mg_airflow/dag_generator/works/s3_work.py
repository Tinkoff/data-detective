...
Модуль содержит класс TS3Work, являющийся наследником TBaseFileWork, реализующий логику работы с
...
from mg_airflow.constants import S3_CONN_ID, WORK_S3_BUCKET, WORK_S3_PREFIX
...
class S3Work(BaseFileWork):
    ...
    def get_size(self, path: str) -> str:
        ...
        size = -1
        if self.exists(path):
            object_summary_response = self.hook.get_resource_type('s3') \
                .ObjectSummary(self.bucket, path).get()
            if 'ContentLength' in object_summary_response:
                size = object_summary_response['ContentLength']
